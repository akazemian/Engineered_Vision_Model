...
target_param = 'LAYER_2_FILTERS'
headers = ['region_dataset',target_param,'mean_score']
...
for param in param_range:
    ...
    print(f'\033[1m' + f'extracting activations for parameter value = {param}' + '\033[0m')
    
    
    model = Model(filters_2=param).Build()
    results_path = os.path.join(path,'Results','Parameter_Study',f'{target_param}.csv')
    ...
    for dataset, regions in brainscore_data_dict.items():
        for region in regions:
            ...
            activations_model = PytorchWrapper(model=model, 
                                        ...
                                        identifier=f'Model-{dataset}-{param}-{target_param}')
    ...
    activations_model = PytorchWrapper(model=model, 
                                preprocessing=PreprocessGS,
                                identifier=f'Model-Object2Vec-{param}-{target_param}')
